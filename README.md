...
Данные маркетплейса шаблонов Chatfuel. Лендинг читает их через `MARKETPLACE_TEMPLATES_BASE_URL` (GitHub raw),
дашборд применяет шаблон через мутацию `fuelyAutomationApplyTemplate`.
...
Каждому значению соответствует фиксированный градиент фона карточки (реализован в коде лендинга, изменить через JSON нельзя):

| Значение | Описание | Градиент фона |
|----------|----------|---------------|
| `cursor` | Фиолетовый | `linear-gradient(-25.39deg, rgb(247,234,255) 184.62%, rgb(94,14,108) 131.69%)` |
